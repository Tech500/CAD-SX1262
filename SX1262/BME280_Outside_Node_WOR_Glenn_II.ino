...
#define EoRa_PI_V1
#include <boards.h>
#include "Gold_II_sx1262_commands.h"
#include <WiFi.h>
#include <ESP32_NOW.h>
#include <esp_now.h>
#include <esp_wifi.h>
...
void releaseRadioPinHolds() {
  gpio_hold_dis((gpio_num_t)RADIO_CS_PIN);
  gpio_hold_dis((gpio_num_t)RADIO_SCLK_PIN);
  gpio_hold_dis((gpio_num_t)RADIO_MISO_PIN);
  gpio_hold_dis((gpio_num_t)RADIO_MOSI_PIN);
  gpio_hold_dis((gpio_num_t)RADIO_BUSY_PIN);
  gpio_deep_sleep_hold_dis();
}

void holdRadioPinsForSleep() {
  gpio_hold_en((gpio_num_t)RADIO_CS_PIN);
  gpio_hold_en((gpio_num_t)RADIO_SCLK_PIN);
  gpio_hold_en((gpio_num_t)RADIO_MISO_PIN);
  gpio_hold_en((gpio_num_t)RADIO_MOSI_PIN);
  gpio_hold_en((gpio_num_t)RADIO_BUSY_PIN);
...
// ============================================================
// SX1262 FULL CONFIG (cold boot only)
// ============================================================

void sxFullConfig()
{
  Serial.println("\n--- SX1262 Gold II full configuration ---");
  ...
  sxCommand(SX126X_CMD_SET_REGULATOR_MODE, regMode, 1);
  ...
  // Required once
  sxSetBufferBaseAddress(0x00, 0x00);

  // IRQ routing for WOR
  sxConfigureRxDutyCycleIrq();

  // Start WOR listen mode
  sxSetRxDutyCycle(RXDC_RX_TICKS, RXDC_SLEEP_TICKS);

  sxClearIrq();

  Serial.println("SX1262 Gold II configuration complete.");
}

// ============================================================
// SX1262 MINIMAL CONFIG -- diagnostic build (cold boot only)
//
// Follows AN1200.36 Section 5 (Usage)'s reference sequence
// exactly. Deliberately OMITS the XTAL trim (REG_XTAL_TRIM_A/B)
// and RX gain (REG_RX_GAIN) register writes present in
// sxFullConfig() -- those were carried over from the CAD-era
// test8D driver and are not part of Section 5's minimal example,
// nor are they touched anywhere in RadioLib's own working
// startReceiveDutyCycle()/begin() path (confirmed against
// RadioLib's SX126x.cpp source). Testing whether these
// unverified CAD-era values are interfering with Listen-mode
// reception on this specific board's crystal.
//
// TEMP: swapped in for sxFullConfig() at both call sites while
// diagnosing the no-wake issue. If this resolves it, decide
// whether XTAL trim/RX gain should be reintroduced with
// board-verified values, or dropped permanently.
// ============================================================

void sxMinimalConfig()
{
  Serial.println("\n--- SX1262 MINIMAL configuration (Gold II, diagnostic) ---");
  ...
  // ------------------------------------------------------------
  // LoRa PHY (SF7 / BW125 / CR4/5)
  // ------------------------------------------------------------
  sxSetFrequency(LORA_FREQ_HZ);
  sxSetLoRaModulation();

  // ------------------------------------------------------------
  // Packet params (implicit header, preamble=2, payload=0xFF)
  // ------------------------------------------------------------
  sxSetPacketParams();

  // ------------------------------------------------------------
  // Buffer base addresses (required once)
  // ------------------------------------------------------------
  sxSetBufferBaseAddress(0x00, 0x00);

  // ------------------------------------------------------------
  // IRQ routing for WOR (DIO1 = RX_DONE)
  // ------------------------------------------------------------
  sxConfigureRxDutyCycleIrq();
  ...
  Serial.println("SX1262 MINIMAL configuration complete.");
}
...
void armAndSleep() {
  Serial.println("Arming RxDutyCycle...");
  ...
  // MUST CLEAR IRQ AFTER SETTING DUTY CYCLE TO PURGE STARTUP GLITCHES
  sxClearIrq(); 
...
void enterLowPowerWOR() {
  ...
  Serial.printf("DIO1 GPIO %d pre-sleep level = %d | IRQ = 0x%04X\n",
                WAKEUP_PIN, digitalRead(WAKEUP_PIN), irqStatus);
