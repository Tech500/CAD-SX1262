...
//   RX_DONE ONLY
//
// TIMEOUT remains visible in IRQ STATUS for diagnostics,
// but does NOT wake the ESP32-S3.
//
// ============================================================
...
void sxConfigureRxDutyCycleIrq()
{
  ...
  // DIO1 IRQ MASK
  //
  // ONLY RX_DONE wakes ESP32-S3.
  // ----------------------------------------------------------

  uint16_t dio1IrqMask =
      IRQ_RX_DONE;
...
// TEST 10B
//
// RX period   = 1050 RTC ticks  (~16.4 ms)
// SLEEP period = 5120 RTC ticks (~80.0 ms)
//
// RTC tick = 15.625 us
//
// The SX1262 autonomously performs:
//
//     RX -> SLEEP -> RX -> SLEEP ...
//
// ESP32-S3 does NOT participate in this loop.
// ============================================================

#define RXDC_RX_TICKS       1050UL
#define RXDC_SLEEP_TICKS    5120UL

void sxSetRxDutyCycle()
{
  uint8_t data[6];

  // RX period - 24 bit
  data[0] = (RXDC_RX_TICKS >> 16) & 0xFF;
  data[1] = (RXDC_RX_TICKS >> 8)  & 0xFF;
  data[2] =  RXDC_RX_TICKS        & 0xFF;

  // Sleep period - 24 bit
  data[3] = (RXDC_SLEEP_TICKS >> 16) & 0xFF;
  data[4] = (RXDC_SLEEP_TICKS >> 8)  & 0xFF;
  data[5] =  RXDC_SLEEP_TICKS        & 0xFF;
